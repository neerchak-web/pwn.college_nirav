...
### Solve
**Flag:** `pwn.college{AFIeYpZzTbsi2QVNJCp19hUwEz3.QXxcjM1wSNyMzNzEzW}`

In this level we will be allowed to use `chgrp` without root access. We can use `chgrp` to change the group ownership of `/flag` to the current user's group (`hacker`).

```
chgrp hacker /flag
cat /flag
pwn.college{AFIeYpZzTbsi2QVNJCp19hUwEz3.QXxcjM1wSNyMzNzEzW}
```
### New Learnings
