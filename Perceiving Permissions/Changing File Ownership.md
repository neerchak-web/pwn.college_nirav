...
### Solve
**Flag:** `pwn.college{EESLwtyMgnEFEXEIhAo4l2UdZat.QXxEjN0wSNyMzNzEzW}`

In this level we will be allowed to use `chown` without root access. We can use `chown` to change the owner of `/flag` to the current user `hacker`.

```
chown hacker /flag
cat /flag
pwn.college{EESLwtyMgnEFEXEIhAo4l2UdZat.QXxEjN0wSNyMzNzEzW}
```
### New Learnings

`chown` command is used to change ownership of files. The first argument is the new owner username, and the second is the file address.
