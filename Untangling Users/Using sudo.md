...
### Solve
**Flag:** `pwn.college{8C8dxmVvJlKLk9hZJLrrkln2EHy.QX4UDN1wSNyMzNzEzW}`

In this level we will have `sudo` access. We can use the `sudo` command to become root and access the flag from `/flag`.

```
sudo cat /flag
pwn.college{8C8dxmVvJlKLk9hZJLrrkln2EHy.QX4UDN1wSNyMzNzEzW}
```
### New Learnings

`sudo` command is used to give root access to users. It doesn't take any arguments or passwords, it gives access based on policies in `/etc/sudoers`.
